...
data_slow = load('import_data/handshaking_slow_results.mat');
data_fast = load('import_data/handshaking_fast_results.mat');
...
[rot_gt_slow, ext_gt_slow, sup_gt_slow] = one_trajectory(all_length_slow, 100);
[rot_gt_fast, ext_gt_fast, sup_gt_fast] = one_trajectory(all_length_fast, 50);
...
% Repeat
rot_gt_slow = zeros(1, 2*traj_repetitions*floor(all_length_slow));
ext_gt_slow = repmat(ext_gt_slow, [1,traj_repetitions]);
sup_gt_slow = repmat(sup_gt_slow, [1,traj_repetitions]);

rot_gt_fast = zeros(1, 2*traj_repetitions*floor(all_length_fast));
...
xlabel('Tilt angle (deg)'); xlim([55, 102]);  ylim([1, 6000]); grid on
...
% save('handshaking_GT_decay_slow.mat', 'kurt_cut_gt_slow');
% save('handshaking_watch_decay_slow.mat', 'kurt_cut_watch_slow');
% save('handshaking_GT_decay_fast.mat', 'kurt_cut_gt_fast');
% save('handshaking_watch_decay_fast.mat', 'kurt_cut_watch_fast');
%% Saturation error
% figure;
...
function [rot_gt, ext_gt, sup_gt] = one_trajectory(all_length, val)
    % Rotation 
    rot_gt = zeros(1, all_length*2);

    % Extension
    ext_gt = [0:-90/floor(all_length-val):-90, -90*ones(1, val)];
    ext_gt = [ext_gt(1:end-1), flip(ext_gt)];
    ext_gt = ext_gt(1:end-1);
    
    % Supination
    sup_gt = [90*ones(1,all_length-val), 90:-(90-63)/val:63];
    sup_gt = [sup_gt(1:end-1), flip(sup_gt)];
    sup_gt = sup_gt(1:end-1);
end
