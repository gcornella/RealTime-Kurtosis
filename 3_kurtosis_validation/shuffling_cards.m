...
data_slow = load('import_data/shuffling_cards_slow_results.mat');
data_fast = load('import_data/shuffling_cards_fast_results.mat');
...
xlabel('Tilt angle (deg)'); xlim([0, 180]);  ylim([1, 800]); grid on
...
%%
% Find the data envelope 
% Smoothed data using 'loess' method
% Remove NaN values
clean_data = kurt_cut_gt_slow(~isnan(kurt_cut_gt_slow));
[up,lo] = envelope(clean_data(300:end), 1);
...
% save('shuffling_cards_GT_decay_slow.mat', 'kurt_cut_gt_slow');
% save('shuffling_cards_watch_decay_slow.mat', 'kurt_cut_watch_slow');
% save('shuffling_cards_GT_decay_fast.mat', 'kurt_cut_gt_fast');
% save('shuffling_cards_watch_decay_fast.mat', 'kurt_cut_watch_fast');
...
function [] = line_histogram(H, color, linestyle, width)
    x = [min(H.BinEdges)+2.5:(max(H.BinEdges)-min(H.BinEdges))/36:max(H.BinEdges)-2.5];
    yq = movmean(H.Values, 2);
