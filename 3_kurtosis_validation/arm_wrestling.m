...
data_slow = load('import_data/arm_wrestling_slow_results.mat');
data_fast = load('import_data/arm_wrestling_fast_results.mat');
...
sup_gt_slow = zeros(1, 2*traj_repetitions*floor(all_length_slow));

rot_gt_fast = 90*ones(1, 2*traj_repetitions*floor(all_length_fast));
ext_gt_fast = repmat(ext_gt_fast, [1,traj_repetitions]);
sup_gt_fast = zeros(1, 2*traj_repetitions*floor(all_length_fast));
...
xlabel('Tilt angle (deg)'); xlim([0, 103]);  ylim([1, 700]); grid on
...
% save('arm_wrestling_GT_decay_slow.mat', 'kurt_cut_gt_slow');
% save('arm_wrestling_watch_decay_slow.mat', 'kurt_cut_watch_slow');
% save('arm_wrestling_GT_decay_fast.mat', 'kurt_cut_gt_fast');
% save('arm_wrestling_watch_decay_fast.mat', 'kurt_cut_watch_fast');
