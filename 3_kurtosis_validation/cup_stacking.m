...
data_slow = load('import_data/cup_stacking_slow_results.mat');
data_fast = load('import_data/cup_stacking_fast_results.mat');
...
sup_gt_slow = 52*ones(1, 2*traj_repetitions*floor(all_length_slow));

rot_gt_fast = repmat(rot_gt_fast, [1,traj_repetitions]);
ext_gt_fast = repmat(ext_gt_fast, [1,traj_repetitions]);
sup_gt_fast = 52*ones(1, 2*traj_repetitions*floor(all_length_fast));
...
xlabel('Tilt angle (deg)'); xlim([40, 80]);  ylim([1, 3000]); grid on
...
% save('cup_stacking_GT_decay_slow.mat', 'kurt_cut_gt_slow');
% save('cup_stacking_watch_decay_slow.mat', 'kurt_cut_watch_slow');
% save('cup_stacking_GT_decay_fast.mat', 'kurt_cut_gt_fast');
% save('cup_stacking_watch_decay_fast.mat', 'kurt_cut_watch_fast');
...
hold on; plot(0:0.01:2*pi, 0:0.01:2*pi, 'k')
...
function [rot_gt, ext_gt, sup_gt] = one_trajectory(all_length)
    ...
    % this shoulkd be 169
    c = length(ext_gt); % -1 to account for the differences
    ext_gt = [ext_gt, -90*ones(1,c)]; % add some stationary values at -90º
    %rot_gt = [rot_true, zeros(1,20)]; % add zeros to replicate the delay
    % make the ext_gt the same size as the others
    original_array = ext_gt;
    original_x = 1:numel(original_array);   % Define the original x values (indices)
    new_x = linspace(1, numel(original_array), length(rot_gt)); % Define the new x values (indices) for interpolation
    interpolated_array = interp1(original_x, original_array, new_x);% Interpolate the original array to the new size
    ext_gt = interpolated_array;
    
    % Supination
    sup_gt = 52*ones(1, length(rot_gt));
end
